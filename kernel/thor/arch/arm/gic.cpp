...
void GicDistributor::Pin::setGroup_(uint8_t group) {
	size_t regOff = (irq_ / 32) * 4;
	size_t bitOff = irq_ & 31;
	...
	auto v = arch::scalar_load_relaxed<uint32_t>(parent_->space_,
			dist_reg::irqGroupBase + regOff);

	v &= ~(1 << bitOff);
	v |= uint32_t(group) << bitOff;

	arch::scalar_store_relaxed<uint32_t>(parent_->space_,
			dist_reg::irqGroupBase + regOff, v);
}
...
void GicCpuInterface::init() {
	...
	for (int i = 0; i < 16; i++)
		infoLogger() << "thor: iface no = " << frg::hex_fmt{dist_->getCurrentCpuIfaceNo_()} << frg::endlog;
