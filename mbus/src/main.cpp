...
async::detached doServeRemoteLane(helix::UniqueLane conversation, std::shared_ptr<Entity> entity) {
	auto [pullLane] =
		co_await helix_ng::exchangeMsgs(
			conversation,
...
async::detached serve(helix::UniqueLane lane) {
	while(true) {
		...
		if(preamble.id() == bragi::message_id<managarm::mbus::GetPropertiesRequest>) {
			...
			HEL_CHECK(sendResp.error());
		} else if(preamble.id() == bragi::message_id<managarm::mbus::ServeRemoteLaneRequest>) {
			auto req = bragi::parse_head_only<managarm::mbus::ServeRemoteLaneRequest>(recvHead);
			auto entity = getEntityById(req->id());
			if(!entity) {
				managarm::mbus::ServeRemoteLaneResponse resp;
				resp.set_error(managarm::mbus::Error::NO_SUCH_ENTITY);
			...
			doServeRemoteLane(std::move(conversation), std::move(entity));
		}else{
